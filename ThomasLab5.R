...
boot1
signif1 <- mean(boot1$t > 0) # about half the T:s are larger than the original one
paste("P-value of non-parametric bootstrap: ",signif1) # P-level
...
nonrandgen <- function(data,alpha){
  ...
  nonrandY <- max(0,min(alpha * data$Day_of_year + beta, 366))
  return(nonrandY)
}
...
hist(pvals, breaks=20,main=c("histogram of Permutation test p-values"))
plot(alpha,pvals, type="l",main=c("Permutation test p-value of","non-random data vs alpha value"),
     xlab="alpha",ylab="p-value")


hist(data2$Price,breaks = 20,main="Histogram of home prices in Albuquerque 1993",xlab="most likely 100s of USD")
...
paste("Bootstrap mean house price bias-correction estimate",round(mean(data2$Price) - mean(boot2$t),3)," hundred USD")
paste("Bias-corrected bootstrap estimate of mean house price: ", round(biascorrectedest,3)," hundred USD")
...
plot(boot3)
paste("Bootstrap estimate of variance of mean house price: ", round(mean(boot3$t),3)," hundred USD squared")
...
paste("Estimate of variance of mean home price by jackknife: ", round((varbyjack$jack.se)^2,3)," hundred USD squared")
...
confintinfos <- data.frame(method = c("Normal Approximation","bootstrap percentile method","BCa method"),
...
confintinfos #as you go down the list the interval creeps higher,also normal approx midpoint is same as bias corrected bootstrap est.
